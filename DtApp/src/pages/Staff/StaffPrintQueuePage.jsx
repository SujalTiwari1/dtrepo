        ...
        {jobs.map((job) => (
          <div key={job.id} className={styles.jobCard}>
            <div className={styles.jobDetails}>
              <h3>Job ID: <span className={styles.jobId}>{job.id}</span></h3>
              <p><strong>Student:</strong> {job.studentEmail}</p>
              <p><strong>File:</strong> <a href={job.fileUrl} target="_blank" rel="noopener noreferrer">{job.fileName}</a></p>
              <p><strong>Preferences:</strong> {job.preferences.copies} copies, {job.preferences.color}, {job.preferences.sided}</p>
